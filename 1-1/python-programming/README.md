...
주차별 파일에는 출력과 자료형, 조건문, 반복문, 컬렉션, 문자열, 함수, 파일 입출력과 객체지향 관련 예제가 포함되어 있습니다. `week-09`에는 Tkinter 예제가, `week-14`에는 Pygame 벽돌깨기 소스가 있습니다.
...
2~3주차 폴더에는 출력 서식, 진법 변환, 지폐 단위 계산, 비트 시프트와 학점 판정 파일이 있습니다.
...
4~7주차 폴더에는 `for`, `while`, `break`, `continue`, 리스트·딕셔너리와 문자열을 사용하는 파일이 있습니다.
...
9~11주차 폴더에는 Tkinter 위젯, 함수 반환값과 사용자 모듈, 텍스트 파일 읽기·쓰기·복사, 문자 이동 방식의 파일 암호화 예제가 있습니다.
...
13주차 폴더에는 자동차 클래스를 사용하는 생성자, 클래스 변수와 상속 예제가 있습니다. 14주차 폴더에는 Pygame 이벤트 루프와 충돌 판정을 사용하는 벽돌깨기 소스가 있습니다.
